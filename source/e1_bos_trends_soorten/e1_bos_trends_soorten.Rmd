...
# Voorspelde waarden berekenen voor de grafiek
pred_ml <- ggpredict(ml, terms = c("fPeriode")) %>%
  mutate(x = recode(x, "1" = "VBI 1", "2" = "VBI 2")) %>%
  mutate(ellenberg = "Licht", sign = "**")
pred_ms <- ggpredict(ms, terms = c("fPeriode")) %>%
  mutate(x = recode(x, "1" = "VBI 1", "2" = "VBI 2")) %>%
  mutate(ellenberg = "Stikstof", sign = "**")
pred_mr <- ggpredict(mr, terms = c("fPeriode")) %>%
  mutate(x = recode(x, "1" = "VBI 1", "2" = "VBI 2")) %>%
  mutate(ellenberg = "Zuurtegraad", sign = "")
pred_mt <- ggpredict(mt, terms = c("fPeriode")) %>%
  mutate(x = recode(x, "1" = "VBI 1", "2" = "VBI 2")) %>%
  mutate(ellenberg = "Temperatuur", sign = "")
pred_mv <- ggpredict(mv, terms = c("fPeriode")) %>%
  mutate(x = recode(x, "1" = "VBI 1", "2" = "VBI 2")) %>%
  mutate(ellenberg = "Vocht", sign = "")
pred_msr <- ggpredict(msr, terms = c("Rand")) %>%
  mutate(x = recode(x, "1" = "VBI 1", "2" = "VBI 2")) %>%
  mutate(ellenberg = "Stikstof \n rand", sign = "**")

pred_m <- base::rbind(pred_ml, pred_ms, pred_msr, pred_mr, pred_mt, pred_mv)

pred_ellenberg <- pred_m %>%
  mutate(ellenberg = recode(ellenberg,
    "Stikstof \n rand" = "Stikstof rand"
  )) %>%
  filter(ellenberg %in% c("Stikstof", "Stikstof rand"))
```

Het aantal boomsoorten per bostype is toegenomen (Figuur 1), maar het aantal kruidachtigen niet (Figuur 2). Van die kruidachtigen neemt het aantal oud-bosplanten wel toe, zowel in oude als in meer recente bossen (Figuur 3).
