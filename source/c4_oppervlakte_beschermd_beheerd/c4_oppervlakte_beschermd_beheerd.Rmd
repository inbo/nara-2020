...
gegevens <- read_vc("bescherming_beheer") %>%
  mutate(Categorie = factor(Categorie,
    ordered = TRUE,
    levels = c("rest", "alleen juridisch beschermd", "juridisch beschermd en
               beheerd")
  )) %>%
...
```{r staafdiagram, fig.width = 10, fig.height = 4, fig.cap = "Oppervlakteaandeel met alleen juridische bescherming en aandeel dat bovendien ook natuurgericht beheerd wordt voor Vlaanderen en per ecosysteemgroep. (*) Bij de rivieren gaat het om het lengteaandeel. Deze figuur verwijst naar figuur 25 in het Natuurrapport 2020."}
gegevens <- gegevens %>%
  group_by(Ecosysteem) %>%
