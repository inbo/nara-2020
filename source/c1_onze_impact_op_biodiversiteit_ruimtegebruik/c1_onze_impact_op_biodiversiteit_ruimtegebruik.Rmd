...
landgebruikgegevens <- landgebruikgegevens %>%
  ...
  group_by(NUTS1regio) %>%
  mutate(prop = Oppervlakte / sum(Oppervlakte) * 100) %>%
  mutate(Oppervlakte = round(x = Oppervlakte, digits = 0)) %>%
  ungroup()
```
...
```{r staafdiagram ecosysteemvoorraad-benchmark, echo =  FALSE, fig.width = 8, fig.height = 4, fig.cap = "Figuur 1. Ruimtegebruik en ecosysteemvoorraad in Vlaanderen en 9 andere sociaal-economische topregio's in 2018 (Bron: Corine, Eurostat). Deze figuur verwijst naar Figuur 8 in het Natuurrapport 2020.", fig.pos = "H"}
test <- function(x) formattable::percent(x, 0) / 100
colorx <- c("black", "black", "black", "black", "black", "black", "black",
            "black", "black", inbo_hoofd)

p <- ggplot(landgebruikgegevens, aes(x = NUTS1regio, y = prop,
                                     fill = Landgebruik)) +
  ...
  scale_fill_manual(
    breaks = c("Stedelijk_bebouwd_gebied", "Akker_grasland", "Natuur",
               "Zoetwater_estuarium", "Zee"),
    values = c("Zee" = inbo_grijsblauw, "Zoetwater_estuarium" = inbo_lichtblauw,
               "Natuur" = inbo_groen, "Akker_grasland" = inbo_geelgr,
               "Stedelijk_bebouwd_gebied" = inbo_rood),
    labels = c("Stedelijk en bebouwd gebied", "Akker en grasland",
               "Terrestrische natuur", "Zoetwater en estuarium", "Zee")
  ) +
  theme(
    axis.title = element_text(family = "Helvetica", size = 18,
                              margin = margin(t = 0, r = 0, b = 0, l = 0),
                              hjust = 0),
    axis.text.y = element_text(family = "Helvetica", size = 18, color = colorx),
...
```{r relatie urbanisatie-bevolkingsdichtheid, echo =  FALSE, fig.width = 8, fig.height = 4, fig.cap = "Figuur 2. Bevolkingsdichtheid en urbanisatie in Vlaanderen en 9 andere sociaal-economische topregio's in 2018 (Bron: Corine, Eurostat). Deze figuur verwijst naar Figuur 9 in het Natuurrapport 2020.", fig.pos = "H"}
test <- function(x) formattable::percent(x, 0) / 100
...
```{r relatie urbanisatie-welvaart, echo =  FALSE, fig.width = 8, fig.height = 4, fig.cap = "Figuur 3. Economische welvaart  en urbanisatie in Vlaanderen en 9 andere sociaal-economische topregio's in 2018 (Bron: Corine, Eurostat).", fig.pos = "H"}
test <- function(x) formattable::percent(x, 0) / 100
labelregio <- c("Comunidad de Madrid", "Hessen", "Nordrhein-Westfalen",
                "Oost-Nederland", "Ostösterreich", "Southeast England",
                "Vlaams Gewest", "West-Nederland", "Zuid-Nederland",
