...
ptn <- ptn[, -(5:8)]
ptn.shp <- spTransform(ptn, CRS(SRS_string = "EPSG:4326"))
```
...
Via de foutenmatrix kunnen ten slotte een aantal accuraatheidsmaten worden berekend, waaronder de *overall accuracy* (OA) en de *producers accuracy* (PA) [@olofsson_good_2014 - zie [script analyse](D1_validatie-landgebruik.Rmd)].
...
palptn <- colorFactor(palette = c("green", "red"), domain = ptn.shp$type)
...
Uit de analyse van deze punten (zie [script analyse](D1_validatie-landgebruik.Rmd)) blijkt dat de overeenstemming tussen de evaluatoren hoog is voor de beoordeling van de stabiliteit van het landgebruik (91% overeenstemming), maar heel wat lager is voor de beoordeling van het type landgebruiksverandering (73% overeenstemming).
