...
Indicatoren helpen het biodiversiteitsbeleid te onderbouwen. De focus van de huidige indicatorenset ligt vooral op oppervlakte- of voorraadindicatoren en soortindicatoren. Indicatoren voor genetische diversiteit ontbreken.
...
ggplot(indicatorgegevens1, aes(x = kwadrant, y = aantal, fill = Schaal)) +
  ...
  geom_text(
    data = indicatorgegevens3, aes(y = lab.ypos, label = aantal), size = 6,
    color = "black"
  ) +
  annotate(
    geom = "text", x = 2.5, y = 40, hjust = 0, size = 6, label = "40",
    color = inbo_rood
  ) +
  annotate(
    geom = "text", x = 2.5, y = 60, hjust = 0, size = 6, label = "60",
    color = inbo_rood
  ) 
```
...
Elk organisatieniveau kan vanuit vier complementaire ecologische invalshoeken bestudeerd worden [@demolder_h_hoofdstuk_2014; @noss_indicators_1990; @pereira_essential_2013] (zie Figuur): 
