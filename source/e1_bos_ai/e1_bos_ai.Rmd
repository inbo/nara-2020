...
pred_mdw <- ggpredict(mdw,
  terms = c("fPeriode", "StandType"),
  type = "fe.zi"
) %>%
  mutate(x = recode(x, "1" = "VBI 1", "2" = "VBI 2")) %>%
...
names(labels) <- c(
  "Boomlaag", "Bosstructuur", "Dood hout",
  "Kruidlaag", "Totaal"
)
...
p <- ggplot(pred_m, aes(
  ...
  facet_wrap(~index,
    scales = "free_y", labeller = labeller(index = labels),
    nrow = 1
  )
