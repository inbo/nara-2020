...
trait_coverage <- function(trait_name, threshold) {
  ...
  a <- master_data %>%
    ...
    mutate(BasalAreaxTrait01 = .data$Species_BasalArea_ha * .data$Trait01) %>%
    group_by(.data$ID, .data$IDPlots, .data$Periode, .data$Total_basal_area) %>%
    mutate(
      Sum_BasalAreaxTrait01 = sum(.data$BasalAreaxTrait01),
      !!name := .data$Sum_BasalAreaxTrait01 / .data$Total_basal_area * 100
    ) %>%
    ungroup()
  b <- a %>%
  ...
  c <- b %>%
    dplyr::select(.data$ID, .data$IDPlots, .data$Periode, !!name) %>%
    group_by(.data$ID, .data$IDPlots, .data$Periode) %>%
    summarise(!!sym(name) := mean(!!sym(name)))
  d <- c %>% # aantal plots per periode met coverage groter dan threshold
    group_by(.data$Periode) %>%
...
cwm_continuous_trait <- function(data, trait_name) {
  ...
  data %>%
    na.omit() %>%
    group_by(.data$ID, .data$IDPlots, .data$Periode) %>%
    mutate(Total_basal_area_adjusted = sum(.data$Species_BasalArea_ha)) %>%
    mutate(traitxBA = .data$Species_BasalArea_ha * !!sym(trait_name)) %>%
    group_by(
      .data$ID, .data$IDPlots, .data$Periode, .data$Total_basal_area_adjusted
    ) %>%
    summarise(sumtraitxBA = sum(.data$traitxBA)) %>%
    mutate(!!name := .data$sumtraitxBA / .data$Total_basal_area_adjusted) %>%
    ungroup()
}
