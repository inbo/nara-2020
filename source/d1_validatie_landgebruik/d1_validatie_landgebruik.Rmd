...
combine <- read_vc("combine") # Combine van de validatieklassenkaart van 2013 en 2016 -> geeft de oppervlakte van de landgebruiksveranderingen en van de stabiele klassen
...
# OPMAAK DATASET -----------------------------------------------------
# De dataset met validatiepunten bevat informatie over de verandering van een cel (0/1) en de aard van de verandering (klasse A -> klasse B). Voor de validatie werden achteraf een aantal moeilijk te onderscheiden klassen samengenomen. De analyse kan dus op 3 niveaus uitgevoerd worden: (1) verandering - geen verandering, (2) originele landgebruiksklassen en (3) de geaggregeerde klassen. In het onderstaande script worden deze drie validatiesets apart aangemaakt.
...
points <- punten %>%
  ...
  # Check of de beoordeling "change/nochange" overeenkomt met die van de LG-kaart
  ungroup() %>%
  group_by(objectid) %>%
  mutate(n = n() / 3) %>%
  ungroup() %>%
  ...
  # Aanpassen beoordeling "verandering" -> als de validatieklasse 2 x hetzelfde is per evaluator, dan "nochange"
  rowwise() %>%
...
# Alleen de verandering ON_W werd gevalideerd. In 38 van de 40 gevalideerde punten verandert er niets (water blijft water), de andere 2 punten blijken ook onveranderlijk (O_O en ON_ON). Hieruit besluiten we dat de kaart de klasse ON_W heel slecht weergeeft en dat we niet van een significante verandering kunnen spreken. Voor de rest van de validatie laten we de waterklassen volledig weg.
...
points_lu <- points_lu %>%
  filter(!str_detect(changeclass, "W"), !str_detect(changeclassref, "W")) %>% # alles met water in weglaten
  droplevels()
...
# VALIDATIE -------------------------------------------------------
# Methode Olofsson et al., 2014 - Code uit het FAO tool (d'Annunzio et al. 2016)
# https://github.com/openforis/accuracy-assessment/blob/master/Rscripts/error_matrix_analysis.R
...
validation <- function(ma, maparea) {
  ...
  v_oa <- sum(propmaparea^2 * ua * (1 - ua) / (ni - 1), na.rm = TRUE) # variance of overall accuracy (Eq. 5 in Olofsson et al. 2014)
...
# save(ov_val, file = "./data/Analyse_validatieklassen.Rdata") # Analyse met validatieklassen (geaggreg. LG-klassen)
# save(ov_change, file = "./data/Analyse_change.Rdata") # Analyse met change - no change
# save(ov_lu, file = "./data/Analyse_lu.Rdata") # Analyse met originele landgebruiksklassen
...
# eval <- points %>%
#   dplyr::select(objectid, verandering, lu2013, lu2016, klasse, oordeel, eval, n) %>%
#   filter(n >= 2 & n < 3) %>% # selecteer punten die 2 keer geëvalueerd werden. Eval = nr evaluator
#   rename("verandmap" = "verandering", "lu13map" = "lu2013", "lu16map" = "lu2016") %>%
#   spread(key = klasse, value = oordeel) %>%
#   rename("verandref" = "verandering", "lu13ref" = "lu2013", "lu16ref" = "lu2016") %>%
#   left_join(dplyr::select(tbltrans, valid, lu), by = c("lu13map" = "lu")) %>% rename(lu13mapval = valid) %>% # validatieklassen toevoegen (= aggregatie van oorspronkelijke lu-klassen)
#   left_join(dplyr::select(tbltrans, valid, lu), by = c("lu16map" = "lu")) %>% rename(lu16mapval = valid) %>%
#   left_join(dplyr::select(tbltrans, valid, lu), by = c("lu13ref" = "lu")) %>% rename(lu13refval = valid) %>% # validatieklassen toevoegen (= aggregatie van oorspronkelijke lu-klassen)
#   left_join(dplyr::select(tbltrans, valid, lu), by = c("lu16ref" = "lu")) %>% rename(lu16refval = valid)
#
# eval_perid <- eval %>%
#   group_by(objectid) %>%
...
#   mutate(verand_klasse = ifelse(lu2013 == TRUE & lu2016 == TRUE, TRUE, FALSE)) %>%
#   mutate(verand_klasseval = ifelse(lu2013val == TRUE & lu2016val == TRUE, TRUE, FALSE)) %>%
...
#   summarise(n = n(), verand = sum(verand[TRUE]), verand_klasse = sum(verand_klasse[TRUE]))
#
# tbl_evalval <- eval_perid %>% # Voor de geaggregeerde klassen
#   group_by(lu13mapval, lu16mapval) %>%
#   summarise(n = n(), verand = sum(verand[TRUE]), verand_klasse = sum(verand_klasseval[TRUE]))
#
## Originele klassen
# totaal <- tbl_eval %>%
#   ungroup() %>%
